...
try {
    $sqlVac = "
        ...
        WHERE v.status = 'active' 
          AND (v.available_until IS NULL OR v.available_until >= :sel_date)
    ";
    $stmtVac = $pdo->prepare($sqlVac);
    $stmtVac->execute([':sel_date' => $selectedDateStr]); // เทียบว่าวันหมดเขตต้องมากกว่าหรือเท่ากับวันที่เลือกจอง
